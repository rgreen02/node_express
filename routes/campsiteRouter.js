      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            //Can modify comments. Not the author
            err = new Error(
              "You can not update comments because your are not the author",
            );
            err.status = 403;
            return next(err);
          }
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error("You must be the author of the comment to delete.");
            err.status = 403;
            return next(err);
          }
        } else if (!campsite) {
